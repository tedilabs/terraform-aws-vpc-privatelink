...
variable "name" {
  description = "(Required) Desired name for the VPC Resource Endpoint."
  type        = string
  nullable    = false
}

variable "resource_configuration" {
  description = "(Required) The ARN (Amazon Resource Name) of the VPC Lattice Resource Configuration to connect the endpoint to. The resource configuration can be owned by your account or shared with your account through AWS RAM."
  type        = string
  nullable    = false
}
...
variable "ip_address_type" {
  description = "(Optional) The type of IP addresses used by the subnets for the resource endpoint. The possible values are `IPv4`, `IPv6` and `DUALSTACK`. Defaults to `IPv4`"
  type        = string
  default     = "IPv4"
  nullable    = false
...
variable "default_security_group" {
  description = <<EOF
  (Optional) The configuration of the default security group for the resource endpoint. `default_security_group` block as defined below.
